...
//                         </div>
//                         <div className="space-y-3 text-sm mb-6">
//                             <p><strong className="text-gray-400 w-24 inline-block">Director:</strong> {movie.directorimage.join(', ')}</p>
//                             <p><strong className="text-gray-400 w-24 inline-block">Cast:</strong> {movie.castimage.join(', ')}</p>
...
//                                     download
//                                     className="bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-6 rounded-full flex items-center gap-2 transition-transform hover:scale-105"
//                                 >
    ...
    return (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
            ...
            >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 p-6">
                    <div className="md:col-span-1">
                        <img src={movie.poster} alt={movie.title} className="rounded-lg w-full mb-4" />
                        
                        {movie.directorimage && movie.directorimage.length > 0 && (
                            <div className="mb-6">
                                <h3 className="text-white font-bold text-lg mb-3">Director{movie.directorimage.length > 1 ? 's' : ''}</h3>
                                <div className="flex flex-wrap gap-3">
                                    {movie.directorimage.map((img, index) => (
                                        <div key={index} className="flex flex-col items-center">
                                            <div className="w-20 h-20 rounded-full overflow-hidden bg-gray-700 flex items-center justify-center">
                                                {img ? (
                                                    <img 
                                                        src={img} 
                                                        alt={movie.director?.[index] || `Director ${index + 1}`} 
                                            ...
                                            {movie.director?.[index] && (
                                                <span className="text-white text-xs mt-2 text-center max-w-[200px] truncate">
                                                    {movie.director[index]}
                                                </span>
                                            )}
                                        </div>
                        ...
                        {movie.castimage && movie.castimage.length > 0 && (
                            <div>
                                <h3 className="text-white font-bold text-lg mb-3">Cast</h3>
                                <div className="flex flex-wrap gap-3">
                                    {movie.castimage.map((img, index) => (
                                        <div key={index} className="flex flex-col items-center">
                                            <div className="w-20 h-20 rounded-full overflow-hidden bg-gray-700 flex items-center justify-center">
                                                {img ? (
                                                    <img 
                                                        src={img} 
                                                        alt={movie.cast?.[index] || `Cast ${index + 1}`} 
                                                        className="w-full h-full object-cover"
                                                    />
                                                ) : (
                                                    <i className="fas fa-user text-primary text-5xl"></i>
                    ...
                    <div className="md:col-span-2">
                        ...
                        <div className="space-y-3 text-sm mb-6">
                            <p><strong className="text-gray-400 w-24 inline-block">Director:</strong> {movie.director?.join(', ') || 'N/A'}</p>
                            <p><strong className="text-gray-400 w-24 inline-block">Cast:</strong> {movie.cast?.join(', ') || 'N/A'}</p>
                            <p><strong className="text-gray-400 w-24 inline-block">Language:</strong> {movie.language}</p>
                            <p><strong className="text-gray-400 w-24 inline-block">Views:</strong> {movie.views?.toLocaleString() || '0'}</p>
                        </div>
