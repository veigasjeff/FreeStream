...
export default function SchedulePage() {
  ...
  const ITEMS_PER_BATCH = isMobile ? 6 : 12; // Smaller batches for mobile
  ...
  useEffect(() => {
    ...
    const initialData = schedule.shows.slice(0, initialCount);
    setVisibleShows(initialData);
    indexRef.current = initialCount;
    
    if (schedule.shows.length <= initialCount) {
      setHasMore(false);
    }
  }, [isMobile]);

  // --- LOAD MORE FUNCTION (Optimized for mobile) ---
  const loadMoreItems = useCallback(() => {
    if (isLoading || !hasMore) return;
    ...
    requestAnimationFrame(() => {
      ...
      if (nextBatch.length > 0) {
        ...
        setVisibleShows(prev => {
          const newShows = [...prev, ...nextBatch];
          return newShows;
        });
        indexRef.current = nextCount;
      }
  ...
  }, [isLoading, hasMore, ITEMS_PER_BATCH]);
  ...
  useEffect(() => {
    ...
    // Mobile-specific intersection observer settings
    const options = {
      root: null,
      rootMargin: isMobile ? '200px' : '100px', // Load earlier on mobile
      threshold: isMobile ? 0.1 : 0.05 // More sensitive on mobile
    };

    const handleIntersection = (entries) => {
      const [entry] = entries;
      if (entry.isIntersecting && hasMore && !isLoading) {
        // Use setTimeout to prevent blocking main thread on mobile
        setTimeout(() => {
          loadMoreItems();
        }, 100);
      }
  ...
  }, [loadMoreItems, hasMore, isLoading, isMobile]);
  ...
  useEffect(() => {
    ...
    const handleTouchMove = (e) => {
      ...
      // If user is within 300px of bottom on mobile, trigger load
      if (distanceFromBottom < 300 && hasMore && !isLoading) {
        isScrolling = true;
        loadMoreItems();
  ...
  }, [isMobile, hasMore, isLoading, loadMoreItems]);
  ...
  // --- MANUAL LOAD MORE BUTTON (Mobile Fallback) ---
  ...
  return (
    <>
      ...
      <section 
        ref={containerRef}
        className="min-h-screen py-12 bg-gradient-to-b from-dark to-black overflow-hidden"
      >
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            <h1 className="text-4xl md:text-5xl font-display font-bold mb-6">
            ...
            <div className="mt-4 text-gray-400 text-lg md:text-2xl">
              Showing {visibleShows.length} of {schedule.shows.length} Movies, Tv Show & Sports Events
            </div>
            ...
            {isMobile && hasMore && (
              <div className="mt-6">
                <button
                  ...
                  disabled={isLoading}
                  className="bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white font-bold py-3 px-6 rounded-lg transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isLoading ? (
              ...
              <div key={date} className="bg-gray-800/50 backdrop-blur-sm border border-gray-700 rounded-xl p-4 md:p-6 animate-fadeIn">
                <div className="mb-6 md:mb-8">
                  ...
                  <p className="text-gray-400 text-sm md:text-base">
                    Live streaming schedule is for this day & Streaming Now.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                  {shows.map((show) => (
                    <div 
                      key={show.id} 
                      className="bg-gray-900 rounded-lg overflow-hidden hover:transform hover:scale-[1.02] transition-transform duration-300"
                    >
                      <div className="relative h-56 md:h-64 overflow-hidden">
                        <img 
                          src={getImageUrl(show.image)}
                      ...
                      <div className="p-4 md:p-6">
                        ...
                        <div className="flex flex-wrap items-center gap-2 md:gap-3 mb-3 md:mb-4">
                          ...
                          <div className="flex items-center gap-1 text-gray-400 text-sm md:text-base">
                            <FaFilm className="text-green-500" />
                            <span>{Array.isArray(show.genre) ? show.genre.join(", ") : show.genre}</span>
                          </div>
                        </div>
          ...
          <div 
            ref={loaderRef}
            className="py-8 md:py-12 flex flex-col items-center justify-center min-h-[100px]"
          >
            {isLoading && hasMore && (
              <div className="flex flex-col items-center gap-3">
                <FaSpinner className="animate-spin text-3xl text-blue-500" />
                <span className="text-gray-400 text-sm">
                  Loading more content... {visibleShows.length} of {schedule.shows.length} shown
                </span>
              </div>
            )}
            
            {!hasMore && visibleShows.length > 0 && (
              <div className="text-center p-6 bg-gradient-to-r from-green-600/20 to-emerald-600/20 rounded-lg border border-green-500/30">
                <p className="text-green-400 font-bold text-lg mb-2">🎉 All Content Loaded!</p>
                <p className="text-gray-400 text-sm">
                  You've viewed all {schedule.shows.length} shows in our schedule
                </p>
              </div>
            )}
          </div>

          {/* MOBILE LOAD MORE BUTTON (Visible when near bottom) */}
          {isMobile && hasMore && !isLoading && (
            <div className="sticky bottom-6 z-10 px-4">
              <div className="bg-gradient-to-r from-blue-600/90 to-purple-600/90 backdrop-blur-sm rounded-xl p-4 shadow-2xl">
                <button
                  onClick={handleManualLoadMore}
                  className="w-full bg-white hover:bg-gray-100 text-gray-900 font-bold py-3 rounded-lg transition-all duration-300 flex items-center justify-center gap-2"
                >
                ...
                <p className="text-white/80 text-xs text-center mt-2">
                  {visibleShows.length} of {schedule.shows.length} shows loaded
                </p>
              </div>
            </div>
          )}
