...
//     // Prepare article content - combine description and first part of content
//     const articleContent = movie.description + ' ' + (movie.content ? movie.content.substring(0, 500) + '...' : '');
    
//     // Format dates for schema
//     const datePublished = formatSchemaDate(movie.date || dateString);
//     const dateModified = formatSchemaDate(movie.date || dateString);
    
//     // Keywords array
//     const keywordsArray = movie.keywords ? movie.keywords.split(',').slice(0, 10).map(k => k.trim()) : [];
...
//             "@type": "Organization",
//             "name": "FreeStream",
//             "url": "https://freestreaming.vercel.app"
//         },
//         "publisher": {
...
//             }
//         },
//         "articleBody": "${escapeJson(articleContent)}",
...
//         "director": ${JSON.stringify(movie.director.map(d => ({ "@type": "Person", "name": escapeJson(d) })))},
//         "actor": ${JSON.stringify(movie.cast.map(actor => ({ "@type": "Person", "name": escapeJson(actor) })))},
//         "aggregateRating": {
...
//     <!-- ORIGINAL STYLE FROM ATTACHMENT FILE -->
//     <style>
//         /* Reset */
//         * {
//             margin: 0;
...
//                     <div>${escapeHtml(movie.language)}</div>
//                 </div>
//             </div>
...
// console.log('✅ Complete NewsArticle Schema.org implementation');
// console.log('✅ Complete Open Graph and Twitter Card meta tags');
// console.log('✅ Enhanced Schema.org structured data (Movie, NewsArticle, BreadcrumbList)');
...
// console.log('\n📰 NEWSARTICLE SCHEMA INCLUDES:');
// console.log('✅ mainEntityOfPage with WebPage type');
// console.log('✅ headline, description, and articleBody');
// console.log('✅ datePublished and dateModified in ISO format');
// console.log('✅ author and publisher with Organization type');
// console.log('✅ image with ImageObject type and dimensions');
// console.log('✅ keywords from movie data');
// console.log('✅ isAccessibleForFree set to True');
...
movies.forEach(movie => {
    const posterUrl = fixPosterUrl(movie.poster);
    const movieYear = extractYear(movie.date);
    ...
    <url>
        ...
        <image:image>
            ...
            <image:caption>${escapeXml(movie.description.substring(0, 200))}</image:caption>
        </image:image>`;
    ...
    if (movie.playUrl) {
        ...
        <video:video>
            <video:thumbnail_loc>${posterUrl}</video:thumbnail_loc>
            <video:title>${escapeXml(movie.title)}</video:title>
            <video:description>${escapeXml(movie.description)}</video:description>
            <video:content_loc>https://www.youtube.com/watch?v=${movie.playUrl}</video:content_loc>
            <video:player_loc allow_embed="yes">https://www.youtube.com/embed/${movie.playUrl}</video:player_loc>
            <video:duration>7200</video:duration>
            <video:publication_date>${new Date(movie.timestamp * 1000).toISOString()}</video:publication_date>
...
movies.forEach(movie => {
    const pubDate = new Date(movie.timestamp * 1000);
    const formattedDate = pubDate.toISOString().split('T')[0];
    const keywords = movie.keywords ? 
        movie.keywords.split(',').slice(0, 10).map(k => k.trim()).join(', ') : 
    ...
    <url>
        ...
        <news:news>
            ...
            <news:publication_date>${formattedDate}</news:publication_date>
            <news:title>${escapeXml(movie.title)}</news:title>
...
fs.writeFileSync('news-sitemap.xml', newsSitemap);
console.log('✅ news-sitemap.xml created - FIXED AND VALID');
